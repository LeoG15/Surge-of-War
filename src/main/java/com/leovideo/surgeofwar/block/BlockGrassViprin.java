...
public class BlockGrassViprin extends ElementsSurgeofWar.ModElement {
	@GameRegistry.ObjectHolder("surgeofwar:grassviprin")
	public static final Block block = null;
	public BlockGrassViprin(ElementsSurgeofWar instance) {
		super(instance, 42);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0, new ModelResourceLocation("surgeofwar:grassviprin", "inventory"));
	}
	...
	public void generateWorld(Random random, int chunkX, int chunkZ, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("surgeofwar:viprinbiome")))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
		for (int i = 0; i < 20; i++) {
	...
	public static class BlockCustomFlower extends BlockFlower {
		public BlockCustomFlower() {
			...
			setResistance(0F);
			setLightLevel(0F);
			setUnlocalizedName("grassviprin");
			setRegistryName("grassviprin");
		}
