...
public class BlockGalzanMushroomBlock extends ElementsSurgeofwarMod.ModElement {
	@GameRegistry.ObjectHolder("surgeofwar:galzan_mushroom_block")
	public static final Block block = null;
	public BlockGalzanMushroomBlock(ElementsSurgeofwarMod instance) {
		super(instance, 208);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("surgeofwar:galzan_mushroom_block", "inventory"));
	}

	@Override
	public void generateWorld(Random random, int chunkX, int chunkZ, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		boolean dimensionCriteria = false;
		if (dimID == WorldGalzan.DIMID)
			dimensionCriteria = true;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(chunkX, 128, chunkZ));
		if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("surgeofwar:galzan_biome")))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
		for (int i = 0; i < 2; i++) {
	...
	public static class BlockCustomFlower extends BlockReed {
		public BlockCustomFlower() {
			...
			setResistance(0F);
			setLightLevel(0.15F);
			setUnlocalizedName("galzan_mushroom_block");
			setRegistryName("galzan_mushroom_block");
		}
