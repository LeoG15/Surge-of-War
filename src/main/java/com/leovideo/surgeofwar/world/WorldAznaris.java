...
import com.leovideo.surgeofwar.item.ItemAznaris;
import com.leovideo.surgeofwar.block.BlockAznarisOmam;
...
public class WorldAznaris extends ElementsSurgeofwarMod.ModElement {
	public static int DIMID = 4;
	public static final boolean NETHER_TYPE = false;
	@GameRegistry.ObjectHolder("surgeofwar:aznaris_portal")
	...
	public void initElements() {
		...
		elements.items.add(() -> new ItemAznaris().setUnlocalizedName("aznaris").setRegistryName("aznaris"));
	}
	...
	public void preInit(FMLPreInitializationEvent event) {
		if (DimensionManager.isDimensionRegistered(DIMID)) {
			...
			System.err.println("Dimension ID for dimension aznaris is already registered. Falling back to ID: " + DIMID);
		}
		dtype = DimensionType.register("aznaris", "_aznaris", DIMID, WorldProviderMod.class, true);
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(ItemAznaris.block, 0, new ModelResourceLocation("surgeofwar:aznaris", "inventory"));
	}
					...
					for (int l7 = 1; l7 < 3; ++l7) {
						for (int k8 = -1; k8 < 3; ++k8) {
							...
							this.world.setBlockState(new BlockPos(k9, k10, k11),
									flag ? BlockAznarisOmam.block.getDefaultState().getBlock().getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
			}
				...
				for (int l8 = 0; l8 < 4; ++l8) {
					for (int l9 = -1; l9 < 4; ++l9) {
						...
						this.world.setBlockState(new BlockPos(l10, l11, k12),
								flag1 ? BlockAznarisOmam.block.getDefaultState().getBlock().getDefaultState() : iblockstate, 2);
					}
				}
	...
	public static class BlockCustomPortal extends BlockPortal {
		public BlockCustomPortal() {
			setHardness(-1.0F);
			setUnlocalizedName("aznaris_portal");
			setRegistryName("aznaris_portal");
			setLightLevel(0F);
		}
		...
		public static class Size {
			...
			protected int getDistanceUntilEdge(BlockPos p_180120_1_, EnumFacing p_180120_2_) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.isEmptyBlock(this.world.getBlockState(blockpos).getBlock())
							|| this.world.getBlockState(blockpos.down()).getBlock() != BlockAznarisOmam.block.getDefaultState().getBlock()) {
						break;
					}
				}
				Block block = this.world.getBlockState(p_180120_1_.offset(p_180120_2_, i)).getBlock();
				return block == BlockAznarisOmam.block.getDefaultState().getBlock() ? i : 0;
			}
			...
			protected int calculatePortalHeight() {
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (block != BlockAznarisOmam.block.getDefaultState().getBlock()) {
								break label56;
							}
				...
				for (int j = 0; j < this.width; ++j) {
					if (this.world.getBlockState(this.bottomLeft.offset(this.rightDir, j).up(this.height)).getBlock() != BlockAznarisOmam.block
	...
	public static class ChunkProviderModded implements IChunkGenerator {
		private static final IBlockState STONE = BlockAznarisDirt.block.getDefaultState();
	...
	public static class GenLayerBiomesCustom extends GenLayer {
		private Biome[] allowedBiomes = {Biome.REGISTRY.getObject(new ResourceLocation("surgeofwar:aznarisbiome")),};
