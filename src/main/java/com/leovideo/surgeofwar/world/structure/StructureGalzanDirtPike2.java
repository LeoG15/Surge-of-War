...
package com.leovideo.surgeofwar.world.structure;
...
public class StructureGalzanDirtPike2 extends ElementsSurgeofwarMod.ModElement {
	public StructureGalzanDirtPike2(ElementsSurgeofwarMod instance) {
		super(instance, 321);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (dimID == WorldGalzan.DIMID) {
			...
			isNetherType = WorldGalzan.NETHER_TYPE;
		}
		...
		if ((random.nextInt(1000000) + 1) <= 10000) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("surgeofwar", "galzan_dirt_pike_2"));
				if (template == null)
					return;
