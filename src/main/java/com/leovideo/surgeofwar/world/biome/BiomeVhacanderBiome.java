...
public class BiomeVhacanderBiome extends ElementsSurgeofwarMod.ModElement {
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Vhacander Biome").setRainfall(0.5F).setBaseHeight(0.1F).setWaterColor(-26215).setHeightVariation(0.2F)
					.setTemperature(0.5F));
	...
	static class CustomTree extends WorldGenAbstractTree {
		...
		public boolean generate(World world, Random rand, BlockPos position) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == BlockVhacandergrass.block.getDefaultState().getBlock()
							|| ground == BlockVhacanderDirt.block.getDefaultState().getBlock())
							&& (ground2 == BlockVhacandergrass.block.getDefaultState().getBlock()
									|| ground2 == BlockVhacanderDirt.block.getDefaultState().getBlock())))
						return false;
					IBlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						world.setBlockState(position.down(), BlockVhacanderDirt.block.getDefaultState(), 2);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getBlock().isLeaves(state, world, blockpos)
												...
												|| state.getBlock() == BlockVhacanderleaves.block.getDefaultState().getBlock()) {
											this.setBlockAndNotifyAdequately(world, blockpos, BlockVhacanderleaves.block.getDefaultState());
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							if (state.getBlock().isAir(state, world, genhPos) || state.getBlock() == Blocks.AIR.getDefaultState().getBlock()
									|| state.getBlock() == BlockVhacanderleaves.block.getDefaultState().getBlock()) {
								this.setBlockAndNotifyAdequately(world, position.up(genh), BlockViprinBlockPortal.block.getDefaultState());
							}
						}
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == BlockViprinBlockPortal.block.getDefaultState().getBlock()
					|| blockType == BlockVhacanderleaves.block.getDefaultState().getBlock()
					|| blockType == BlockVhacandergrass.block.getDefaultState().getBlock()
					|| blockType == BlockVhacanderDirt.block.getDefaultState().getBlock();
		}

		@Override
		protected void setDirtAt(World world, BlockPos pos) {
			if (world.getBlockState(pos).getBlock() != BlockVhacanderDirt.block.getDefaultState().getBlock())
				this.setBlockAndNotifyAdequately(world, pos, BlockVhacanderDirt.block.getDefaultState());
		}
